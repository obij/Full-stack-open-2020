...
const Blog = ({ blog,  user, addLike, deleteBlog, postedByArr }) => {
  ...
  return(
    <div style={blogStyle}>
      {showBlogDetails === false ? <span>{blog.title} {blog.author}<button onClick= {() => setShowBlogDetails(true)}>view</button></span> 
                               : <div><span>{blog.title} {blog.author} <button onClick= {() => setShowBlogDetails(false)}>hide</button></span> <BlogDetails blog={blog}  user= {user} updateLike={updateLike}  removeBlog= {removeBlog} postedByArr={postedByArr}/> </div>
      }
    </div>
  )
}
